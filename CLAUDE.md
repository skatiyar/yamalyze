...
- **`lib.rs`** — WASM entry point. Shared `validate_and_parse()` handles empty-input checks and `serde_yml` parsing with simultaneous error reporting (prefixed `[YAML ONE]`/`[YAML TWO]` with line numbers). Exports a single diff entry point:
  - `compute_diff(yone, ytwo)` — parses both inputs, computes the full diff tree via `yaml_diff()`, and returns a `JsValue` array (serialized via `diff_vec_to_js`). Handles all top-level types: mappings, sequences, scalars, and mixed.
- **`diff.rs`** — Recursive diff engine. Internal types `YamlDiff`, `DiffType`, `DiffValue` are `pub(crate)` (not exported to WASM) and serialized to plain JS objects before crossing the WASM boundary (via `diff_node_to_js`/`diff_vec_to_js` using `js-sys`), eliminating per-getter cloning overhead.
  ...
  - `DiffValue` — holds `left_value` and `right_value` as `serde_yml::Value`
  - Core functions: `map_diff()` (objects), `seq_diff()` (arrays via Myers diff), `val_diff()` (scalars), `yaml_diff()` (dispatcher)
  - `DiffValue::new()` and `YamlDiff::new()` — `pub(crate)` constructors
  - Helper functions: `unwrap_tagged()` (strips `Value::Tagged` wrappers — serde_yml wraps version-like strings like `3.0.1` in Tagged, `pub(crate)`), `to_js()` (safe serialization via `serde_wasm_bindgen` with Tagged unwrapping, `pub(crate)`), `yaml_key_to_string()` (non-string key support, `pub(crate)`), `serialize_value()` (canonical string for Myers comparison), `value_to_diff_children()` (recursive child tree generation for pure additions/deletions — complex values become expandable instead of flat `{}`/`[...]`), `diff_node_to_js()`/`diff_vec_to_js()` (plain JS object serialization, `pub(crate)`)
  - Types derive `serde::Serialize` in test builds (`#[cfg_attr(test, ...)]`) for `insta` snapshot assertions
...
- **`index.js`** — Imports `compute_diff` from `../pkg` (generated by wasm-pack). Key features:
  - Async diff — `runDiff` is async. Calls `compute_diff(v1, v2)` for the full diff in one WASM call. Concurrency guard (`diffInProgress`/`diffPending`) prevents overlapping diffs.
  - Stage-based loader — diff container shows a spinner with stage text ("Computing diff...", "Rendering...") during processing
...
- **`style.css`** — Tailwind CSS v4 with custom classes for editor gutter (`.editor-gutter`, `.gutter-line--error`), stacked overlay container (`.editor-overlays` — absolute bottom, flex column), error overlay (`.editor-error` — red), storage warning overlay (`.editor-warning` — amber), per-panel size warning overlay (`.editor-size-warning` — translucent amber), diff run button (`.diff-run-btn`), diff loader (`.diff-loader`, `.diff-loader-spinner` — CSS animate-spin with dark mode), diff tree (`.diff-row--addition`, `.diff-row--deletion`, `.diff-row--modified`, leaf row alignment via invisible `::before` arrow placeholder on `.diff-node > .diff-row` and `.diff-row.diff-node-root`), diff filter buttons (`.diff-filter`, `.diff-filter--active`), collapsible details/summary, and file upload button.
...
`index.html` is the template processed by HtmlWebpackPlugin. It contains the page layout with a header (logo + tagline), two side-by-side editor panels (each with a line number gutter, textarea, file upload button, and a `.editor-overlays` stacking container at the bottom holding size warning, storage warning, and error overlays), a diff results container with a "Run Diff" button (hidden by default, shown for large files), clickable filter buttons in the summary bar, a loader with spinner and stage text, a collapsible diff tree, a placeholder message, a WebAssembly fallback error, and a footer with privacy note, size limit, creator link, and GitHub icon. Includes SEO meta tags, Open Graph/Twitter cards, and JSON-LD structured data.
